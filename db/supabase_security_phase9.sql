...
-- Account deletion audit (written by service role; readable by super admins)
...
create index if not exists account_deletion_audit_deleted_at_idx on public.account_deletion_audit(deleted_at desc);
create index if not exists account_deletion_audit_user_id_idx on public.account_deletion_audit(user_id);
create index if not exists account_deletion_audit_email_idx on public.account_deletion_audit(lower(email));

alter table public.account_deletion_audit enable row level security;

drop policy if exists account_deletion_audit_super_select on public.account_deletion_audit;
create policy account_deletion_audit_super_select
